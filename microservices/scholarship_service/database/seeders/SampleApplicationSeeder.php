...
class SampleApplicationSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // These will be linked to existing citizen users from auth service
        // User ID 200: john.doe@email.com / password123
        // User ID 201: jane.smith@email.com / password123
        // User ID 202: michael.johnson@email.com / password123
        $sampleData = [
            [
                'user_id' => 200, // Links to John Doe (john.doe@email.com)
                'student' => [
                    'first_name' => 'John',
                    'middle_name' => 'M.',
                    'last_name' => 'Doe',
                    'student_id_number' => 'STU-2024-001',
                    'email_address' => 'deployone73@gmail.com',
            ...
            [
                'user_id' => 201, // Links to Jane Smith (jane.smith@email.com)
                'student' => [
                    'first_name' => 'Jane',
                    'middle_name' => 'L.',
                    'last_name' => 'Smith',
                    'student_id_number' => 'STU-2024-002',
                    'email_address' => 'mahusaycchristian2@gmail.com',
            ...
            [
                'user_id' => 202, // Links to Michael Johnson (michael.johnson@email.com)
                'student' => [
                    'first_name' => 'Michael',
                    'middle_name' => 'R.',
                    'last_name' => 'Johnson',
                    'student_id_number' => 'STU-2024-003',
                    'email_address' => 'grindshine478@gmail.com',
        ...
        foreach ($sampleData as $data) {
            try {
                // Create or update student linked to existing auth user
                $student = Student::updateOrCreate(
                    ['student_id_number' => $data['student']['student_id_number']], // Find by student ID
                    [
                        'citizen_id' => 'CIT-' . date('Y') . '-' . str_pad(rand(1, 999999), 6, '0', STR_PAD_LEFT),
                        'user_id' => $data['user_id'], // Link to existing auth user
                ...
                $this->command->info("✓ Created student: {$data['student']['first_name']} {$data['student']['last_name']} (User ID: {$data['user_id']})");
            } catch (\Exception $e) {
                $this->command->error("✗ Failed to create student {$data['student']['first_name']} {$data['student']['last_name']}: " . $e->getMessage());
