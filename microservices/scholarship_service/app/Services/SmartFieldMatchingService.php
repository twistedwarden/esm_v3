...
class SmartFieldMatchingService
{
    ...
    protected $fieldDefinitions = [
        'student_id_number' => [
            'variations' => ['student_id', 'id_number', 'student_no', 'id_no', 'stud_id', 'student_code', 'lrn', 'learner_reference_number', 'studentid', 'studentnumber'],
            'weight' => 1.5
        ],
        'first_name' => [
            'variations' => ['given_name', 'fname', 'first', 'given', 'givenname', 'firstname'],
            'weight' => 1.2
        ],
        'last_name' => [
            'variations' => ['surname', 'family_name', 'lname', 'last', 'family', 'familyname', 'lastname'],
            'weight' => 1.2
        ],
        'middle_name' => [
        ...
        'contact_number' => [
            'variations' => ['mobile', 'phone', 'cellphone', 'tel_no', 'cp_no', 'contact_no', 'telephone', 'mobile_no', 'phonenumber', 'mobilenumber', 'contactnumber'],
            'weight' => 1.3
        ],
        'sex' => [
            'variations' => ['gender', 'sex_at_birth', 'biological_sex'],
            'weight' => 1.0
        ],
        'birth_date' => [
            'variations' => ['dob', 'date_of_birth', 'birthday', 'birthdate', 'born_on', 'dateofbirth'],
            'weight' => 1.3
        ],
        'birth_place' => [
            'variations' => ['place_of_birth', 'pob', 'born_at', 'birthplace', 'placeofbirth'],
            'weight' => 1.0
        ],
        'address' => [
            'variations' => ['residence', 'current_address', 'permanent_address', 'location', 'home_address', 'homeaddress', 'currentaddress'],
            'weight' => 1.0
        ],
        'program' => [
            'variations' => ['course', 'degree', 'major', 'strand', 'specialization', 'course_enrolled', 'degreeprogram'],
            'weight' => 1.2
        ],
        'year_level' => [
            'variations' => ['year', 'grade', 'level', 'yr_level', 'grade_level', 'academic_level', 'yearlevel', 'gradelevel'],
            'weight' => 1.2
        ],
        'enrollment_status' => [
            'variations' => ['status', 'is_enrolled', 'registration_status', 'academic_status', 'enrollmentstatus'],
            'weight' => 1.1
        ],
        'enrollment_term' => [
            'variations' => ['term', 'semester', 'sem', 'school_term', 'academic_term', 'enrollmentterm'],
            'weight' => 1.1
        ],
        'school_year' => [
            'variations' => ['academic_year', 'sy', 'ay', 'year', 'academicyear', 'schoolyear'],
            'weight' => 1.1
        ],
    ];
    ...
    public function matchHeaders(array $headers): array
    {
        ...
        foreach ($headers as $header) {
            ...
            foreach ($this->fieldDefinitions as $field => $definition) {
                ...
                if ($score > $highestScore && $score > 0.6) { // Threshold
                    $highestScore = $score;
                    $bestMatch = $field;
                }
            }
